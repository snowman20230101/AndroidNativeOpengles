 ...
 * */
    ...
    bool isCJKUnicode(char32_t ch) {
        return (ch >= 0x4E00 && ch <= 0x9FBF)   // CJK Unified Ideographs
               ...
               || (ch >= 0x2FF0 && ch <=
                                   0x30FF)   // Ideographic Description Characters, CJK Symbols and Punctuation & Japanese
    ...
    std::string getStringUTFCharsJNI(JNIEnv *env, jstring srcjStr, bool *ret) {
        ...
        if (srcjStr != nullptr && env != nullptr) {
            const unsigned short *unicodeChar = (const unsigned short *) env->GetStringChars(
                    srcjStr, nullptr);
    ...
    void StringUTF8::replace(const std::string &newStr) {
        ...
        if (!newStr.empty()) {
            ...
            if (lengthString == 0) {
                LOGD("Bad utf-8 set string: %s", newStr.c_str());
                return;
            }
...
unsigned short *
cc_utf8_to_utf16(const char *str_old, int length/* = -1*/, int *rUtf16Size/* = nullptr*/) {
    if (str_old == nullptr)
        return nullptr;
